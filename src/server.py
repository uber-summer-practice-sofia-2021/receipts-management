...
# server stuff
server = Flask(
    __name__, template_folder="resources/templates", static_folder="resources/"
)
...
def getAllInfo(tripID):
    ...
    orderResponse = requests.post(
        myController.get_order_info, json=courierResponse["orderId"]
    )
...
def receiveTripId():
    ...
    response_from_order = myController.PostReuqestToOrderService(
        {"orderId": response_from_courier.json()["orderId"]}
    )
